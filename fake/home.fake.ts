...
export default defineFakeRoute([
  ...
  },
  {
    ...
    response: ({ body }) => {
      if (body.range === 'week') {
        return Array.from({ length: 7 }).map(() => faker.number.int({ min: 100, max: 1000 }));
      }
      if (body.range === 'month') {
        const currentDate = new Date();
        const currentDay = currentDate.getDate();
        return Array.from({ length: currentDay }).map(() => faker.number.int({ min: 100, max: 1000 }));
      }
      if (body.range === 'year') {
        const currentDate = new Date();
        ...
        return Array.from({ length: daysBeforeCurrentMonth.length }).map(() =>
