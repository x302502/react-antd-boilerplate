...
export function BuiltinTheme() {
	const { builtinTheme, themeColorPrimary, setPreferences } =
		usePreferencesStore();
	const [color, setColor] = useState(
		builtinTheme === "custom" ? themeColorPrimary : "#1677ff"
	);
...
{t("preferences.theme.builtin.title")}
	...
	const handleColorChange: ColorPickerProps["onChangeComplete"] = (
		aggregationColor
	) => {
	...
	function handleClick(value: BuiltinThemeType) {
		setPreferences({
			builtinTheme: value,
			themeColorPrimary: builtinThemePresets.find(
							...
							<dl className="mb-0">
								...
								>
									<div
										...
										style={{
											backgroundColor:
												item.value === "custom" ? color : item.color
										}}
									>
