...
export const NotificationPopup: React.FC<Props> = ({
	...
	return (
				...
				<List
					...
					header={(
						<div className="flex items-center justify-between">
							...
							<Tooltip
								title={
									notifications?.length ? t("widgets.markAllAsRead") : null
								}
							>
								<BasicButton
					...
					renderItem={(item) => (
						...
						>
							{!item.isRead && (
								<span className="bg-primary absolute right-2 top-2 h-2 w-2 rounded"></span>
							)}
							...
							<div className="flex flex-col gap-1 leading-none">
								...
								<p className="text-muted-foreground my-1 line-clamp-2 text-xs">
									{item.message}
								</p>
		...
		>
			...
			>
				{dot && (
					<span className="bg-blue-600 absolute right-2 top-1.5 h-2 w-2 rounded"></span>
				)}
			</BasicButton>
		</Popover>
	);
